...
public class AlxLocationManager implements GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener, LocationListener {
    public static final String GOOGLE_API_KEY = "AIzaSyDZvBUxtop35E_Yb8aYADCDRghYr1gktFQ";
//    public static final String GOOGLE_API_KEY = " AIzaSyDLToSD04V2ylySZaswsUWZX9s-WCGDx5g";
    ...
    private static GpsTask mGpsTask;

    private Context context;//防止内存泄漏，不使用activity的引用
    ...
    private static AlxLocationManager sInstance;

    //是否请求天气成功
    private static boolean isWeatherGetSuc = false;

    public void init(ILocationCallBack callBack) {
        mLocationCallBack = callBack;
    }

    public void setWeatherGet(boolean isGetWeather) {
        isWeatherGetSuc = isGetWeather;
    }

    public boolean getWeatherSuc() {
        return isWeatherGetSuc;
    }
    ...
    public static AlxLocationManager getInstance() {
        if (sInstance == null) {
            synchronized (AlxLocationManager.class) {
                if (sInstance == null) {
                    sInstance = new AlxLocationManager();
                }
            }
        }
        return sInstance;
     ...
     * @param context
     */
    public static void onCreateGPS(final Context context) {
        ...
        new Handler().postDelayed(mGpsTask, 9000);
    }

    private static class GpsTask implements Runnable{
        @Override
        public void run() {
            if (manager == null || manager.context == null || manager.currentStatus != STATUS.NOT_CONNECT || isWeatherGetSuc)
                return;//如果连接GPS硬件到9s后还没成功
...
//            Toast.makeText(manager.context, "警告：你没有安装谷歌服务框架，请root后安装", Toast.LENGTH_LONG).show();
            manager.context.startService(new Intent(manager.context, AlxLocationService.class));
            ...
            new AlxAsynTask<Void, Void, String>() {//在子线程中获取附近的基站和wifi信息
                ...
                protected void onPostExecute(String json) {
                    ...
                    if (manager != null && manager.context != null)
                        manager.sendJsonByPost(json, "address=1600+Amphitheatre+Parkway,+Mountain+View,+CA&key=" + GOOGLE_API_KEY);
                    else return;
//                    Toast.makeText(manager.context, "没有安装谷歌框架！！！发给google api 的json是；：" + json, Toast.LENGTH_LONG).show();
                }
            }.executeDependSDK();
        }
    }
    ...
    public static void pauseGPS() {
        ...
        try {
            ...
            if (manager.locationTimer != null) {
                manager.locationTimer.cancel();
                manager.locationTimer = null;
            }
            if (mGpsTask != null) {
    ...
    public void onConnected(@Nullable Bundle bundle) {
        ...
        if (!getCurrentLocation()) {//得到当前gps并记录
            ...
            try{
                ...
                Log.e("AlexLocation", e.toString()    +  " LocationServices.FusedLocationApi.requestLocationUpdates(mGoogleApiClient, createFastLocationRequest(), this);//创建位置监听");
            }
...
//            Toast.makeText(manager.context, "成功获取到了上次的经纬度，所以不进行追踪了", Toast.LENGTH_LONG).show();
    ...
    class TowerAndWiFiTask extends AlxAsynTask<Void, Void, String> {
        ...
        protected void onPostExecute(String json) {
            ...
            Log.i("AlexLocation", "请求Google位置api：" + url);
            sendJsonByPost(json, url);
//            Toast.makeText(context, "恭喜你安装了谷歌框架，发给google api 的json是；：" + json, Toast.LENGTH_LONG).show();
    ...
    public boolean getCurrentLocation() {
        ...
        try {
            if (mGoogleApiClient != null) {
                ...
                if (isDebugging)
        //            Toast.makeText(context, "获取到last location经纬度  " + "纬度" + latitude + "  经度" + longitude + "精确度" + last_accuracy, Toast.LENGTH_LONG).show();
                ...
                if (last_accuracy < MAX_deviation) {
                    mLocationCallBack.callback(latitude, longitude);
    ...
    //是否拿到位置权限
    public boolean isLocationPerGranted() {

        if (ActivityCompat.checkSelfPermission(JuziApp.getAppContext(), Manifest.permission.ACCESS_FINE_LOCATION) != PackageManager.PERMISSION_GRANTED &&
                ActivityCompat.checkSelfPermission(JuziApp.getAppContext(), Manifest.permission.ACCESS_COARSE_LOCATION) != PackageManager.PERMISSION_GRANTED) {
            // TODO: Consider calling
            //    ActivityCompat#requestPermissions
            // here to request the missing permissions, and then overriding
            //   public void onRequestPermissionsResult(int requestCode, String[] permissions,
    ...
    public void onLocationChanged(Location location) {
        ...
        if (isDebugging)
//            Toast.makeText(context, "获取到了最新的GPS    " + location.toString() + " 精度是" + location.getAccuracy(), Toast.LENGTH_LONG).show();
        ...
        if (location.getAccuracy() > 50 || currentStatus != STATUS.TRYING_FIRST || isGeoApp)
            return;//如果现在是高电量模式，那么就停止当前监听，采用低电量监听
        //地理位置敏感APP并不会进入低电量模式
        mLocationCallBack.callback(location.getLatitude(), location.getLongitude());
    ...
    public static void recordLocation(Context context, double latitude, double longitude, float accuracy){
        Log.i("AlexLocation", "谷歌返回的经纬度是" + latitude + "  :  " + longitude + "  精度是" + accuracy + "recordLocation");
    ...
    public void sendJsonByPost(String json, String url){
        ...
        HttpRequest.post(url, params, new BaseHttpRequestCallback<String>(){
            ...
            protected void onSuccess(String s) {
                ...
                if (isDebugging)
//                    Toast.makeText(context, "谷歌成功，返回json是" + result, Toast.LENGTH_LONG).show();
                ...
                try {
                    ...
                    mLocationCallBack.callback(latitude, longitute);
                    if (isDebugging)
//                        Toast.makeText(context, "谷歌返回的经纬度是" + latitude + "  :  " + longitute + "  精度是" + google_accuracy, Toast.LENGTH_LONG).show();
            ...
            public void onFailure(int errorCode, String msg) {
                ...
                mLocationCallBack.onFailed(errorCode, msg);
                pauseGPS();
