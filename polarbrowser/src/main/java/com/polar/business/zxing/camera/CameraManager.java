...
package com.polar.business.zxing.camera;
...
//    framingRect = new Rect(leftOffset, topOffset, leftOffset + width, topOffset + height);
      ...
      if (screenResolution == null) {
    	  CustomToastUtils.getInstance().showTextToast("相机可能已被禁用");
		return null;
	}
      int width = screenResolution.x * 7 / 10;
...
//      rect.bottom = rect.bottom * cameraResolution.y / screenResolution.y;
  ...
  public void openLight(){
	  if (camera != null) {
			...
			flashModes = parameter.getSupportedFlashModes();
			// Check if camera flash exists
			if (flashModes == null) {
				CustomToastUtils.getInstance().showTextToast(R.string.no_flash_found);
				return;
			}else{
