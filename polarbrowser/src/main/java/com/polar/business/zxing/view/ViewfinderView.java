...
package com.polar.business.zxing.view;
...
public final class ViewfinderView extends View {
	...
	public ViewfinderView(Context context, AttributeSet attrs) {
		...
		lineScan = DensityUtil.dip2px(getContext(), 2);
	}
...
//			canvas.drawRect(hmiddle-1, frame.top+2, hmiddle+2, frame.bottom-1, paint);
			...
			if(currentLineY == 0){
				currentLineY = frame.top;
			}
			currentLineY += lineScan;
			if(currentLineY >= frame.bottom){
				currentLineY = 0;
			}
			
			canvas.drawRect(frame.left + 2, currentLineY - 1, frame.right - 1,
					currentLineY + 2, paint);
